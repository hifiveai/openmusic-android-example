 ...
 * #date: 2022/7/19
 */
public class HiFiveProgress extends View {
    ...
    public HiFiveProgress(Context context) {
        this(context, null);
    }

    public HiFiveProgress(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HiFiveProgress(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        @SuppressLint("Recycle")
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.HiFiveProgress);
        ...
        mBackPaint.setStrokeWidth(typedArray.getDimension(R.styleable.HiFiveProgress_hiFive_backWidth, 2));
        mBackPaint.setColor(typedArray.getColor(R.styleable.HiFiveProgress_hiFive_backColor, 0x33ffffff));
        ...
        mProgPaint.setStrokeWidth(typedArray.getDimension(R.styleable.HiFiveProgress_hiFive_progWidth, 2));
        mProgPaint.setColor(typedArray.getColor(R.styleable.HiFiveProgress_hiFive_progColor, Color.WHITE));

        // 初始化进度圆环渐变色
        int startColor = typedArray.getColor(R.styleable.HiFiveProgress_hiFive_progStartColor, -1);
        int firstColor = typedArray.getColor(R.styleable.HiFiveProgress_hiFive_progFirstColor, -1);
        if (startColor != -1 && firstColor != -1) mColorArray = new int[]{startColor, firstColor};
        else mColorArray = null;

        // 初始化进度
        mProgress = typedArray.getInteger(R.styleable.HiFiveProgress_hiFive_progress, 10);
        typedArray.recycle();
    }
